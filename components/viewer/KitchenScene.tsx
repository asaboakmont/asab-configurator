...
export default function KitchenScene({ cabinets, colorway, wallA, wallB, cornerSide = "right" }: KitchenSceneProps) {
  const midX = (wallA * CM) / 2;
  const tallCabs = cabinets.filter(c =>
    c.wall === "A" && ["tall","tall-oven","tall-fridge"].includes(c.type)
  );
  // Tall cabs sit at RIGHT end of wall A — worktop runs from 0 to start of tall cabs
  const tallStartX = tallCabs.length > 0
    ? Math.min(...tallCabs.map(c => c.xPos))
    : wallA;
  const worktopStartX = 0;
  const worktopEndX = tallStartX * CM;
...
function Room({ wallA, wallB, cornerSide }: { wallA: number; wallB?: number; cornerSide?: string }) {
  ...
  useFrame(() => {
    ...
    // Back wall fades as camera moves behind it
    setBackOpacity(Math.max(0.0, Math.min(0.6, (z / 5) * 0.6)));
    // Side walls fade as camera moves to the sides
  ...
  return (
    <group>
      ...
      <mesh receiveShadow position={[roomW / 2 - 50 * CM, wallH / 2, -0.5 * CM]}>
        ...
        <meshStandardMaterial color="#F0EDE6" roughness={0.9} transparent opacity={backOpacity} />
      </mesh>
      <mesh receiveShadow position={[leftWallX, wallH / 2, roomD / 2 - 50 * CM]}>
        <boxGeometry args={[1 * CM, wallH, roomD]} />
        <meshStandardMaterial color="#E8E4DC" roughness={0.9} transparent opacity={sideOpacity} />
      </mesh>
...
// Preload all known models
const MODEL_SKUS = [
  "1003","1003-HOB-60","1003-HOB-80",
  "1004","1005","1006","1007","1008","1010","1013","1014","1015","1015-45",
  "2004","2007","2008","2010","2013","2014",
  "1011A","1011B","1011C","1016"
];
...
function CabinetMeshGLB({ cabinet, colorway, posX, posY, posZ, rotY }: {
  cabinet: Cabinet; colorway: Colorway;
  posX: number; posY: number; posZ: number; rotY: number;
}) {
  const { sku } = cabinet;
  const [meshes, setMeshes] = React.useState<{geometry: THREE.BufferGeometry; matName: string}[]>([]);
  ...
  const doorMat = useMemo(() =>
    new THREE.MeshStandardMaterial({
      ...
      roughness: colorway.finish === "lucios" ? 0.05 : colorway.finish === "furnir" ? 0.8 : 0.45,
  ...
  const handleMat = useMemo(() =>
    new THREE.MeshStandardMaterial({
      ...
      roughness: colorway.handle === "inox" ? 0.1 : 0.3,
      metalness: colorway.handle === "inox" ? 0.95 : 0.5,
      envMapIntensity: 1.5,
      depthWrite: true,
      side: THREE.FrontSide,
        ...
        loader.parse(
          ...
          (gltf: any) => {
            ...
            gltf.scene.traverse((child: any) => {
              if (child.isMesh) {
                const matName = Array.isArray(child.material) ? child.material[0]?.name : child.material?.name ?? 'carcass';
  ...
  const isWallCabGLB = ["wall","wall-corner","wall-hood"].includes(cabinet.type); // meters — used for mirror center
  return (
    <group position={[posX, posY, posZ]} rotation={[0, rotY, 0]}>
      {(() => {
        // Separate door+handle meshes from carcass meshes
        ...
        return (
          <>
            {carcassMeshes.map((m, i) => (
              <mesh key={"c"+i} geometry={m.geometry} material={matMap[m.matName] ?? carcassMat}
                scale={[10,10,10]} castShadow receiveShadow frustumCulled={false} />
            ))}
            {mirrorDoor ? (
              // Mirror door+handle around cabinet center
              // For wall cabs (isWallCab): mirror X; for base cabs: mirror Z (handle on front face)
              <group
              ...
              >
                <group position={isWallCabGLB ? [-cabW * 10 / 2, 0, 0] : [0, 0, 0]}>
                  {doorMeshes.map((m, i) => (
                    <mesh key={"d"+i} geometry={m.geometry} material={matMap[m.matName] ?? carcassMat}
                      scale={[10,10,10]} castShadow receiveShadow frustumCulled={false} />
                  ))}
                </group>
              </group>
...
function CabinetMesh({ cabinet, colorway, wallA, cornerSide = "right" }: { cabinet: Cabinet; colorway: Colorway; wallA: number; cornerSide?: string }) {
  const { width, height, depth, xPos, wall, type } = cabinet;
  ...
  if (cabinet.wall === "B") {
    if (type === "base-corner" || type === "wall-corner") {
      // Corner cab on Wall B (left wall):
      // After XZ swap: GLB X=0.95m, Z=0.56m
      // Corner on Wall B: back flush to Wall B, face toward user
    ...
    if (type === "base-corner" || type === "wall-corner") {
      // Center-based box positioning (same as regular Wall C cabs)
      posX = wallA * CM - (isWallCab ? WALL_Z : BASE_Z);
      posZ = (width / 2) * CM;  // centered, starting from Wall A (Z=0)
      rotY = -Math.PI / 2;
    } else {
      posX = wallA * CM - (isWallCab ? WALL_Z : BASE_Z);
      posZ = xPos * CM + (width * CM) / 2;
  ...
  const carcassMat = useMemo(() =>
    new THREE.MeshStandardMaterial({ color: colorway.carcassHex, roughness: 0.7 }),
    [colorway.carcassHex]);
  const doorMat = useMemo(() =>
    new THREE.MeshStandardMaterial({
      color: colorway.doorHex,
  ...
  const handleMat = useMemo(() =>
    new THREE.MeshStandardMaterial({
      color: colorway.handleHex,
      roughness: 0.2,
      metalness: colorway.handle === "inox" ? 0.9 : 0.3,
    }), [colorway.handleHex, colorway.handle]);

  const hY = isWallCab ? -(height * CM) * 0.3 : (height * CM) * 0.3;
  const hZ = (depth * CM) / 2 - 0.5 * CM;
  ...
  const handleZ = D / 2 + T / 2;

  const edgeMat = new THREE.MeshStandardMaterial({ color: "#D0CBC0", roughness: 0.9 });
  ...
  if (hasModel) {
    // GLB origin expected: bottom-left-front corner
    ...
    const WALL_OFFSET = 2.0 * CM; // ~2–3cm padding like Wall A
  ...
  return (
    <group position={[posX, posY, posZ]} rotation={[0, rotY, 0]}>
      ...
      {/* Bottom panel */}
      <mesh castShadow receiveShadow material={carcassMat} position={[0, -H/2 + PLINTH_H + T/2, 0]}>
        <boxGeometry args={[W - T*2, T, D]} />
      </mesh>
      {/* Plinth */}
      {!isWallCab && !isTall && (
      ...
      {/* Single door */}
      {hasDoors && numDoors === 1 && (
        <group position={[0, -PLINTH_H/2, D/2 + T/2]} scale={cabinet.doorDirection === "D" ? [-1,1,1] : [1,1,1]}>
          ...
          <mesh material={handleMat}
            position={[doorW/2 - 3.5*CM, handleY - posY + (isWallCab ? (RULES.WALL_CAB_FROM_FLOOR + H/2)*CM : H/2*CM) - posY, 0]}
            rotation={[0, 0, Math.PI/2]}>
            <capsuleGeometry args={[0.3*CM, 12*CM, 4, 8]} />
          </mesh>
        </group>
      )}
      {/* Double doors */}
      {hasDoors && numDoors === 2 && (
        <>
          <group position={[-(doorW/2 + DOOR_GAP/2), -PLINTH_H/2, D/2 + T/2]}>
            ...
            <mesh material={handleMat} position={[cabinet.doorDirection === "D" ? -(doorW/2 - 3*CM) : doorW/2 - 3*CM, handleY, 0]} rotation={[0,0,Math.PI/2]}>
              <capsuleGeometry args={[0.3*CM, 12*CM, 4, 8]} />
            </mesh>
          </group>
          <group position={[doorW/2 + DOOR_GAP/2, -PLINTH_H/2, D/2 + T/2]}>
